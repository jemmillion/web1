...
public class GujikjaCtrl {
	...
	Gujikja register(Scanner sc,Gujikja[] guArr) {
		...
		do {
			...
			for(int i=0;i<Gujikja.count;i++) { // guArr[0]==>"leess" guArr[1]==>"eomjh"
		...
		do {
			...
			if(!MyUtil.isCheckPasswd(passwd)) {
				System.out.println(">> 암호는 8글자 이상 15글자 이하의 대,소문자 및 숫자,특수기호가 혼합되어야 한다.\n");
			}
			else 
				break;
		}while(true);
	...
	// == 구직자 정보를 모두 보기를 해주는 메소드 생성하기 ==
	   ...
	   void showByAgeline(Gujikja[] guArr,int gucount,int ageline) {
		  ...
		  for(int i=0;i<gucount;i++) {
			   ...
			   // 27/10 ==> 2*10 == 20,	25/10*10 ==> 2*10 == 20
